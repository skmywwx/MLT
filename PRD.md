...
| 导入时间展示 | 明信片收纳册中，导入年月日固定显示在拍摄地点与时间**下方**；文件夹模式可在详情中展示完整 `imported_at` |
...
| 元数据展示 | 拍摄日期/时间/地点在照片上方；**导入年月日在拍摄地点与时间下方** | 同上；导入日期字号可略小、颜色略浅以区分层级 |
...
| 拍摄元数据 | 每张照片**上方**：拍摄日期、拍摄时间、拍摄地点（EXIF） |
| 导入时间 | 每张照片**下方**（拍摄地点与时间之后）：导入年月日（`imported_at` 派生） |
