...
- [0. 文档信息](#0-文档信息)
  - [0.1 修订记录](#01-修订记录)
  - [0.2 版本与变动等级](#02-版本与变动等级)
    - [0.2.1 三级变动定义](#021-三级变动定义)
    - [0.2.2 PRD 版本号规则](#022-prd-版本号规则)
    - [0.2.3 index.html 版本号规则](#023-indexhtml-版本号规则)
    - [0.2.4 后续修订规则](#024-后续修订规则)
...
| 文档状态 | Draft |
| 当前版本 | V2.0.1（字体恢复与变动等级规则修正） |
| 时间标准 | 北京时间（UTC+8） |
| 首版生成时间 | 2026-08-13 10:17（北京时间） |
| 最近修订时间 | 2026-08-13 15:01（北京时间） |
| 目标端 | Web 端、桌面端 |
| 目标读者 | 产品、设计、前端、后端、算法、测试、运营 |

### 0.1 修订记录

> 本表已补录此前缺失的 V1.0 首版记录。所有时间均采用北京时间（UTC+8）；后续每次修改 PRD 时，必须同步更新“当前版本”“最近修订时间”及本修订记录。**变动等级**见 [§0.2 版本与变动等级](#02-版本与变动等级)。

| 版本 | 修订时间（北京时间） | 变动等级 | 修订类型 | 修改内容摘要 |
| --- | --- | --- | --- | --- |
| V1.0 | 2026-08-13 10:17 | **重大** | 第一版生成 | 首次整理完整 PRD；定义快速模式与交互模式、用户故事、P0/P1/P2 功能、核心流程、深度研究任务流、文风库、结果展示、跨端一致性、关键字段、异常场景、验收标准及待澄清问题。 |
| V2.0 | 2026-08-13 10:29 | **重大** | 第二版需求补充 | 将每条主任务、阶段任务和子任务流的可视化追踪及百分比进度条标记为 P0；补充进度展示、加权计算、状态颜色、更新频率、暂停/失败/重试/重规划、无障碍、数据字段、异常场景和验收规则。 |
| V2.0-r1 | 2026-08-13 10:32 | 轻微 | 第二版修订记录补录 | 补录此前缺少的版本历史，明确第一版生成时间、第二版修改时间及对应修改内容，并将整份文档的修订时间统一为北京时间；本次不改变产品需求范围。 |
| V2.0-r2 | 2026-08-13 10:37 | 轻微 | 第二版目录整理 | 统一一级、二级、三级和四级标题层次，在文档顶部新增可点击的多级目录，覆盖全部章节及子章节，便于快速定位；本次不改变产品需求范围。 |
| V2.0-r3 | 2026-08-13 10:43 | 轻微 | 第二版双栏阅读布局 | 新增独立双栏阅读页面：左侧较窄区域展示可独立上下滑动的目录，右侧展示可独立上下滑动的 PRD 正文；目录支持点击定位和当前章节高亮，左右区域均常驻显示滚动条；本次不改变产品需求范围。 |
| V2.0-r4 | 2026-08-13 10:57 | 轻微 | 第二版双栏阅读布局优化 | 优化 `PRD.html` 双栏布局比例与滚动体验：左侧目录区收窄为独立滚动面板，右侧正文区独立滚动；两侧均常驻显示滚动条轨道与滑块，目录与正文滚动互不影响；本次不改变产品需求范围。 |
| V2.0-r5 | 2026-08-13 11:03 | 轻微 | PRD 双栏离线同步 | 修复 `PRD.html` 直接打开时无法加载 `PRD.md` 的问题；新增 `scripts/sync-prd-html.py`，将 `PRD.md` 正文内嵌到 `PRD.html`；约定后续修改 PRD 正文后必须同步更新两个文件；本次不改变产品需求范围。 |
| V2.0-r6 | 2026-08-13 11:14 | 轻微 | PRD 字体与结构优化 | 将 `PRD.html` 正文字体整体放大 2 倍并加宽阅读区域；为多级无序/有序列表增加层次化 bullet 样式；在 PRD 正文中补充多级 bullet 与序号结构，强化章节层级表达；本次不改变产品需求范围。 |
| V2.0-r7 | 2026-08-13 11:28 | 轻微 | PRD 字体缩小 | 将 `PRD.html` 阅读页正文字号、标题字号、目录字号及表格/代码块字号整体缩小至上一版的 65%；本次不改变产品需求范围。 |
| V2.0-r8 | 2026-08-13 11:42 | 轻微 | index.html 迭代记录 | 在文档信息区新增 `0.3 index.html 原型迭代记录`，从首版起逐条补录各版本修改时间与变更摘要；本次不改变产品需求范围。 |
| V2.0-r9 | 2026-08-13 13:52 | **中等** | 原型任务流内嵌 | 将 AI 智能助手任务流模拟逻辑从 `scripts/workflow-sim.js` 合并至 `index.html` 单文件，便于双击打开即可完成圆形流程测试；本次不改变产品需求范围。 |
| V2.0-r10 | 2026-08-13 14:00 | **中等** | 原型单脚本整合 | 将任务流模拟与页面交互逻辑合并为 `index.html` 内单一 `<script>`；移除独立沙盒入口与外部脚本依赖，发送需求即在本页进入智能体工作区完成全流程原型测试；本次不改变产品需求范围。 |
| V2.0-r11 | 2026-08-13 14:05 | **中等** | 对话页任务流原型 | 重构发送需求后的对话页布局：可滚动聊天流、负责人交接、内嵌待办与文档预览，底部固定输入框；按用户需求模拟 docx/pptx 交付；本次不改变产品需求范围。 |
| V2.0-r12 | 2026-08-13 14:12 | **中等** | 进度条与智能体职责修复 | 同步 `index.html` V1.19：修复最终交付时进度条应为 100% 的逻辑；明确六智能体按职责参与任务拆分与 handoff；本次不改变产品需求范围。 |
| V2.0-r13 | 2026-08-13 14:22 | **中等** | Logo 返回首页 | 同步 `index.html` V1.20：对话页完成后点击左上角 Logo 返回首页并重置任务流状态；本次不改变产品需求范围。 |
| V2.0-r14 | 2026-08-13 14:29 | **中等** | 智能体团队职责同步 | 新增 §7.0「办公写作智能体团队」，按首页「您的智能体团队」设计稿同步六类智能体的角色定位、能力标签、核心职责与专业价值；与 §7.1 阶段负责人映射对齐。 |
| V2.0-r15 | 2026-08-13 14:32 | 轻微 | PRD 字体放大 | 将 `PRD.html` 阅读页正文字号、标题字号、目录字号及表格/代码块字号整体放大至上一版的 165%（+65%）；本次不改变产品需求范围。 |
| V2.0-r16 | 2026-08-13 14:48 | 轻微 | 变动等级规则 | 新增 §0.2 三级变动等级定义；在 §0.1 与 §0.3 修订表中增加「变动等级」列，并补标注交互体相关条目为**中等**；本次不改变产品需求范围。 |
| V2.0.1 | 2026-08-13 15:01 | 轻微 | 字体恢复与规则修正 | 将 `PRD.html` 字号恢复至 V2.0-r7 水平（撤销 r15 放大）；修正 §0.2 版本号段位：重大 `V1.0→V2.0`、中等 `V1.0→V1.1→V1.2`、轻微 `V1.0→V1.0.1→V1.0.2`；本次不改变产品需求范围。 |

### 0.2 版本与变动等级

PRD 正文修订（§0.1）与 `index.html` 原型迭代（§0.3）采用统一的三级变动等级。**版本号段位与变动等级一一对应**，读版本号即可判断重要程度。

#### 0.2.1 三级变动定义

| 变动等级 | 版本号变化规则 | 示例 | 典型内容 | 是否改变核心需求/主流程 |
| --- | --- | --- | --- | --- |
| **重大** | 递增**主版本号**（第一位） | `V1.0` → `V2.0` → `V3.0` | 新产品方向、模式定义重构、核心流程重写、P0 范围变更、原型主版本升级 | 是 |
| **中等** | 递增**次版本号**（第二位） | `V1.0` → `V1.1` → `V1.2` | 交互体任务流、多智能体 handoff、对话页布局、新功能模块、智能体职责补全、可验收的交互逻辑变更 | 部分（增量或局部） |
| **轻微** | 递增**补丁号**（第三位） | `V1.0` → `V1.0.1` → `V1.0.2` | 字体/字号/间距/排版、错别字、双栏阅读样式、修订记录补录等不影响主内容的变动 | 否 |

> **交互体相关**（快速/交互模式任务流、六智能体协作、进度条与负责人展示、对话页与交付模拟等）属于 **中等**，版本号应递增次版本位，例如 `V2.0` → `V2.1` → `V2.2`。

#### 0.2.2 PRD 版本号规则

| 变动等级 | 版本号写法 | 正确示例 | 错误示例 |
| --- | --- | --- | --- |
| 重大 | `VX.0` 主版本 +1 | `V2.0` → `V3.0` | 用 `V2.0.1` 表示中等功能 |
| 中等 | 次版本 +1 | `V2.0` → `V2.1` → `V2.2` | 用 `V2.0-r9` 或 `V2.0.1` 表示中等功能 |
| 轻微 | 补丁 +1 | `V2.1` → `V2.1.1` → `V2.1.2` | 用 `V2.2` 表示仅改字体 |

说明：

1. **三位版本号**格式为 `主版本.次版本.补丁`（如 `V2.1.3`）。未写补丁位时视为 `.0`（如 `V2.1` = `V2.1.0`）。
2. 历史记录中的 `V2.0-r1` … `V2.0-r16` 为规则建立前使用的修订序号，**保留不 retroactive 重编号**；其中 `r9`–`r14` 实质为**中等**变动，`r1`–`r8`/`r15`–`r16` 为**轻微**变动。
3. 自 **V2.0.1** 起，轻微变动统一使用补丁位（第三位），不再新增 `rN` 序号。
4. 自下一中等变动起，使用次版本号（如 `V2.1` 汇总交互体相关 PRD 同步，或按单次中等改动递增 `V2.2`）。

#### 0.2.3 index.html 版本号规则

| 变动等级 | 版本号写法 | 正确示例 |
| --- | --- | --- |
| 重大 | 主版本 +1 | `V1.20` → `V2.0` |
| 中等 | 次版本 +1 | `V1.0` → `V1.1` → `V1.2` |
| 轻微 | 补丁 +1 | `V1.2` → `V1.2.1` → `V1.2.2` |

说明：

1. 历史 `index.html V1.1` … `V1.20` 保留原序号；其中 **V1.10–V1.20**（登录入口、模式切换、交互任务流等）为**中等**；**V1.1–V1.9** 多为布局/样式微调，变动等级标**轻微**（当时次版本位被误用于补丁级改动，见 §0.3 逐条标注）。
2. 自 **V1.20** 之后：**中等**改动递增至 `V1.21`、`V1.22`；**轻微**改动使用 `V1.21.1`、`V1.21.2`。
3. **交互体 / 任务流 / 多智能体协作** 必须使用次版本位递增，不得占用补丁位。

#### 0.2.4 后续修订规则

1. 每次修改必须新增一条修订记录，不得覆盖或删除历史记录。
2. 修订时间统一采用北京时间（UTC+8），使用 `YYYY-MM-DD HH:mm` 格式。
3. 每条记录必须填写**变动等级**（重大 / 中等 / 轻微），且**版本号段位必须与等级一致**（见 [§0.2.1](#021-三级变动定义)）。
4. **重大** → 主版本 +1；**中等** → 次版本 +1；**轻微** → 补丁 +1。
5. 每条记录需概括新增、修改、删除的内容及主要影响范围。
6. **每次修改 PRD 正文后，必须执行 `python3 scripts/sync-prd-html.py`，将最新内容同步到 `PRD.html`。** `PRD.md` 是唯一编辑源，`PRD.html` 为双栏阅读展示页，不得只改其中一个文件。
7. 若同一次改动同时涉及 PRD 与 `index.html`，两处修订表均需追加记录，且变动等级应一致（除非一侧仅为排版同步，可标轻微并使用补丁位）。

### 0.3 index.html 原型迭代记录

> 本表专门记录办公写作 Agent 原型页 `index.html` 的迭代历史，与 PRD 正文修订记录（0.1）相互独立。所有时间均采用北京时间（UTC+8）；后续每次修改 `index.html` 时，必须在本表末尾新增一条记录，不得覆盖或删除历史记录。**变动等级**见 [§0.2 版本与变动等级](#02-版本与变动等级)。

| 版本 | 修改时间（北京时间） | 变动等级 | 修改内容摘要 |
| --- | --- | --- | --- |
| index.html V1.0 | 2026-08-12 11:40 | **重大** | 首版 HTML 原型：单文件实现办公写作 Agent 首页，含 259px 侧边栏、居中输入框、3 个快捷操作 pill、2×3 智能体卡片；引入 Tailwind CDN、Chart.js、Lucide 图标与 Inter 字体，主色 `#5B5FE5`。 |
| index.html V1.1 | 2026-08-12 11:48 | 轻微 | 内嵌 SVG favicon，完善浏览器标签页图标展示。 |
| index.html V1.2 | 2026-08-12 14:04 | 轻微 | 按修订稿对齐整体布局比例：调整 hero 区、输入框、智能体网格的尺寸与间距，使视觉层级更接近设计稿。 |
| index.html V1.3 | 2026-08-12 14:17 | 轻微 | 微调专家智能体卡片网格的行间距与列间距。 |
| index.html V1.4 | 2026-08-12 15:06 | 轻微 | 优化侧边栏导航选中态：选中项改为灰色背景 + 加粗文字，与 hover 态区分更清晰。 |
| index.html V1.5 | 2026-08-12 15:29 | 轻微 | 新增侧边栏 hover 灰色反馈；最近对话列表支持 hover 显示删除图标并可删除条目。 |
| index.html V1.6 | 2026-08-12 16:03 | 轻微 | 统一附件下拉菜单与删除图标的视觉样式，与整体 UI 规范对齐。 |
| index.html V1.7 | 2026-08-12 16:18 | 轻微 | 侧边栏宽度固定为 259px；附件「+」按钮改为方形圆角样式。 |
| index.html V1.8 | 2026-08-12 16:33 | 轻微 | 侧边栏导航与最近对话字号按比例放大，提升可读性。 |
| index.html V1.9 | 2026-08-12 16:47 | 轻微 | 快捷操作 pill 按类别区分 hover 色：写作为绿色、深度研究为橙色、AI PPT 为紫色。 |
| index.html V1.10 | 2026-08-12 16:59 | **中等** | 新增桌面启动页（模力通图标）与登录页（分栏布局、用户名/密码、密码显隐）；登录成功后进入 Agent 主界面；登录页主色 `#00A0DF`。 |
| index.html V1.11 | 2026-08-12 17:09 | 轻微 | 登录表单支持 Enter 键提交。 |
| index.html V1.12 | 2026-08-12 17:20 | **中等** | 恢复并完善输入区模式选择交互：智能体 / 快速模式 / 文风助手库；快速模式面板可展开切换「快速模式」与「交互模式」，选中态为 cyan 高亮。 |
| index.html V1.13 | 2026-08-12 17:32 | **中等** | 新增个人中心：头像 hover 显示「个人中心」tooltip；右侧 405px 抽屉含账号信息、权益详情、管理中心与退出登录。 |
| index.html V1.14 | 2026-08-12 17:40 | 轻微 | 调整个人中心 tooltip 定位至头像正下方。 |
| index.html V1.15 | 2026-08-13 11:39 | **中等** | 新增交互模式工作流模拟：提交 prompt 后进入「智能体工作区」，按 PRD 深度研究任务流展示进度条、待办事项、多智能体协作消息、工具日志、大纲确认、分章撰写、三审三校、最终交付、「你可能想问」及文件/反馈弹窗；复杂任务自动推荐交互模式。 |
| index.html V1.16 | 2026-08-13 13:52 | **中等** | 将任务流模拟脚本内嵌至 `index.html` 单文件，删除 `scripts/workflow-sim.js`；支持双击打开 HTML 即可完成原型圆形流程测试，无需额外 JS 文件或本地服务。 |
| index.html V1.17 | 2026-08-13 14:00 | **中等** | 合并为单一内嵌脚本；移除「沙盒演示」独立入口，发送需求后直接在 `index.html` 内切换至智能体工作区播放完整任务流，作为原型默认交互路径。 |
| index.html V1.18 | 2026-08-13 14:05 | **中等** | 发送需求后跳转对话页（图2 布局）：可滚动聊天流展示用户消息、负责人交接、内嵌待办、任务拆分与文档预览；底部固定输入框；支持按需求交付 docx/pptx；交互格式对齐图3 完整任务流。 |
| index.html V1.19 | 2026-08-13 14:12 | **中等** | 修复进度条逻辑：中间步骤按加权计算且最高 99%，仅最终交付时显示 100% 与绿色完成态；按图3 六智能体职责拆分交互（统筹规划→检索收集→文献/研究分析→写作撰写→编辑校对→统筹交付）；补全研究员角色；底部流转过程对齐实际 handoff 顺序。 |
| index.html V1.20 | 2026-08-13 14:22 | **中等** | 对话页任务完成后，点击左上角「midu｜模力通」Logo 返回首页输入区（与「新建对话」一致），并重置进度条与聊天流状态。 |

**index.html 维护约定**

1. 每次修改 `index.html` 后，在本表末尾递增版本号并新增一条记录，同时填写**变动等级**。
2. 版本号规则（与 [§0.2.3](#023-indexhtml-版本号规则) 一致）：
   - **重大**：主版本递增（如 `V1.20` → `V2.0`）
   - **中等**：次版本递增（如 `V1.0` → `V1.1` → `V1.2`）；交互体 / 任务流 / 多智能体协作
   - **轻微**：补丁递增（如 `V1.2` → `V1.2.1` → `V1.2.2`）；仅样式、排版、字号微调
3. 历史 V1.1–V1.9 在规则建立前误用次版本位表示轻微改动；表中按**实际内容**标轻微，后续新改动须按段位区分。
4. 修改内容摘要需说明用户可见变化，而非仅列 commit message。
5. 若某次修改同时更新了 PRD 需求范围，PRD 修订记录（0.1）与本表均需分别追加记录，且变动等级应一致。
