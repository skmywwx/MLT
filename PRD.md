...
- [7. 双形态部署：Web 与小程序差异](#7-双形态部署web-与小程序差异)
  - [7.1 账号与个人中心](#71-账号与个人中心)
  - [7.2 照片导入与全部照片库](#72-照片导入与全部照片库)
  - [7.3 主题文件夹与明信片收纳册](#73-主题文件夹与明信片收纳册)
  - [7.4 电子相册编辑（手账式）](#74-电子相册编辑手账式)
  - [7.5 电子相册【预览】拟真翻页](#75-电子相册预览拟真翻页)
  - [7.6 通用差异](#76-通用差异)
- [8. 待进一步澄清的问题](#8-待进一步澄清的问题)
...
> **双形态差异**详见 [§7 双形态部署：Web 与小程序差异](#7-双形态部署web-与小程序差异)。
...
| Q8 | **双端功能是否必须完全对等？** 原型阶段 Web 与小程序是否要求能力一致，还是允许一端先行（如 Web 完整演示、小程序仅收纳+浏览）？ | 排期、资源投入 | 产品 + 研发 |
| Q9 | **双端账号与数据如何打通？** 微信登录的小程序用户能否在 Web 端访问同一账号与作品？本期无云存储时，跨端数据是否暂不考虑同步？ | 账号体系、架构 | 研发 + 产品 |
| Q10 | **小程序本地存储上限下如何约束照片？** 单用户最多导入多少张、单张大小上限多少？是否需自动压缩或引导用户「仅在 Web 端编辑大图」？ | 小程序体验、技术方案 | 研发 + 产品 |
| Q11 | **双端 UI 是否共用一套设计系统？** Web 与小程序是同一套组件/视觉规范适配，还是分端独立设计后再对齐核心流程？ | 设计效率、一致性 | 设计 + 产品 |
| Q12 | **小程序审核与隐私合规边界？** 相册权限、位置信息（拍摄地点）、用户上传图片的 UGC 审核责任方是谁？原型演示是否需正式小程序上线？ | 合规、上线节奏 | 法务 + 研发 |
