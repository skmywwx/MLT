...
| 文档状态 | Draft |
| 当前版本 | V2.0 |
| 首版生成时间 | 2026-08-13 02:17 UTC |
| 最近修订时间 | 2026-08-13 02:31 UTC |
| 目标端 | Web 端、桌面端 |
| 目标读者 | 产品、设计、前端、后端、算法、测试、运营 |

### 0.1 修订记录

> 本表已补录此前缺失的 V1.0 首版记录。后续每次修改 PRD 时，必须同步更新“当前版本”“最近修订时间”及本修订记录。

| 版本 | 修订时间（UTC） | 修订类型 | 修改内容摘要 |
| --- | --- | --- | --- |
| V1.0 | 2026-08-13 02:17 | 首版生成 | 首次整理完整 PRD；定义快速模式与交互模式、用户故事、P0/P1/P2 功能、核心流程、深度研究任务流、文风库、结果展示、跨端一致性、关键字段、异常场景、验收标准及待澄清问题。 |
| V2.0 | 2026-08-13 02:31 | 需求补充 | 将每条主任务、阶段任务和子任务流的可视化追踪及百分比进度条标记为 P0；补充进度展示、加权计算、状态颜色、更新频率、暂停/失败/重试/重规划、无障碍、数据字段、异常场景和验收规则；建立修订记录并补录 V1.0。 |

### 0.2 后续修订规则

1. 每次修改必须新增一条修订记录，不得覆盖或删除历史记录。
2. 修订时间统一使用 `YYYY-MM-DD HH:mm UTC` 格式。
3. 普通内容补充、规则优化或字段调整递增次版本号，例如 V2.0 → V2.1。
4. 产品范围、核心流程或模式定义发生重大变化时递增主版本号，例如 V2.x → V3.0。
5. 每条记录需概括新增、修改、删除的内容及主要影响范围。
6. 仅修正错别字或排版时也应记录，但可标记为“文档修正”。
...
- 目标明确、内容简单、强调交付速度的任务，例如改写标题、生成通知、提炼摘要。
- 信息复杂、需要研究规划、多智能体协作或人工确认的任务，例如行业研究、方案论证、长篇报告。

如果所有任务采用同一套交互方式，简单任务会显得流程过重，复杂任务又会缺少过程透明度和用户控制。因此需要将任务执行方式划分为快速模式与交互模式，并统一文风、研究过程、任务状态和结果展示规则。
...
- 命中“研究、分析、对比、论证、规划、报告”等复杂意图时，推荐交互模式。
- 上传多份文件、要求跨文档引用或开启深度研究时，推荐交互模式。
- 任务预计涉及三个以上子任务或两个以上智能体时，推荐交互模式。
- 改写、扩写、缩写、润色、摘要等单目标任务，推荐快速模式。
- 用户手动选择的模式在当前会话中保持，除非用户再次切换。
...
- 子任务默认权重为 1；系统可根据预计工作量设置权重，但必须在任务详情中可查看。
- `pending` 和 `ready` 状态默认进度为 0%。
- `running` 状态根据智能体上报的已完成节点计算，禁止仅按等待时间虚增进度。
- `waiting_user`、`paused` 和 `failed` 状态沿用进入该状态前的进度。
- `succeeded` 状态固定为 100%；`cancelled` 不计入剩余进度，但总体状态需标记任务计划发生变化。
- 新增、删除或重新拆分子任务时，服务端重新计算总体进度，并向客户端下发变更原因。
...
- 协作任务展示“主负责人 + N 位协作者”，展开后查看完整名单。
- 负责人切换时保留变更记录。
- 执行中任务需明确当前正在工作的负责人。
- 负责人不可用时，状态变为“等待调度”，不得仍显示“执行中”。
...
- 来源数量及可展开的来源列表。
- 与验收规则的匹配情况。
- 复制、引用到正文、下载、重新执行、反馈等操作。

执行失败时不得生成伪结果，应展示失败原因、已完成部分及影响范围。
...
- 切换文风不自动重跑检索与分析任务，只重新执行受影响的撰写和校验任务。
- 结果页需显示当前使用的文风，并支持一键切换后重新生成。
...
- 每次最终结果至少生成 3 个问题，默认最多展示 5 个。
- 至少覆盖以下三类：结论深化、风险或反例、下一步行动。
- 问题不得与用户原始问题完全重复。
- 问题应可由当前结果和已有资料继续回答。
- 点击问题后继承当前任务上下文，并明确使用当前模式还是新建任务。
