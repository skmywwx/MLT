...
| 文档状态 | Draft |
| 当前版本 | V2.0-r8（index.html 迭代记录） |
| 时间标准 | 北京时间（UTC+8） |
| 首版生成时间 | 2026-08-13 10:17（北京时间） |
| 最近修订时间 | 2026-08-13 11:42（北京时间） |
| 目标端 | Web 端、桌面端 |
...
| index.html V1.15 | 2026-08-13 11:39 | 新增交互模式工作流模拟：提交 prompt 后进入「智能体工作区」，按 PRD 深度研究任务流展示进度条、待办事项、多智能体协作消息、工具日志、大纲确认、分章撰写、三审三校、最终交付、「你可能想问」及文件/反馈弹窗；新增 `scripts/workflow-sim.js`；复杂任务自动推荐交互模式。 |

**index.html 维护约定**

1. 每次修改 `index.html`（含关联脚本如 `scripts/workflow-sim.js`）后，在本表末尾递增版本号并新增一条记录。
