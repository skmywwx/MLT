...
| 文档状态 | Draft |
| 当前版本 | V2.0-r16（变动等级规则） |
| 时间标准 | 北京时间（UTC+8） |
| 首版生成时间 | 2026-08-13 10:17（北京时间） |
| 最近修订时间 | 2026-08-13 14:48（北京时间） |
| 目标端 | Web 端、桌面端 |
...
PRD 正文修订（§0.1）与 `index.html` 原型迭代（§0.3）采用统一的三级变动等级，便于区分「需求/交互架构变化」与「排版或细节修正」。

#### 0.2.1 三级变动定义

| 变动等级 | 版本号变化示例 | 典型内容 | 是否改变核心需求/主流程 |
| --- | --- | --- | --- |
| **重大** | `V1.0` → `V2.0` | 新产品方向、模式定义重构、核心流程重写、P0 范围变更、原型主版本升级 | 是 |
| **中等** | `V1.0` → `V1.1` | 交互体任务流、多智能体 handoff、对话页布局、新功能模块、智能体职责补全、可验收的交互逻辑变更 | 部分（增量或局部） |
| **轻微** | `V1.0` → `V1.0.1` | 字体/间距/排版、错别字、修订记录补录、双栏阅读样式、不改变主内容的文档结构优化 | 否 |

> **交互体相关**（快速/交互模式任务流、六智能体协作、进度条与负责人展示、对话页与交付模拟等）统一标注为 **中等**，即使同步记录使用 `rN` 修订号（历史兼容）或 `index.html V1.x` 递增号。

#### 0.2.2 PRD 版本号规则

| 变动等级 | 版本号写法 | 示例 |
| --- | --- | --- |
| 重大 | 递增**主版本号** | `V2.0` → `V3.0` |
| 中等 | 递增**次版本号** | `V2.0` → `V2.1` |
| 轻微 | 递增**修订号** `rN` 或**补丁号** | `V2.0` → `V2.0-r1` / `V2.0.1` |

说明：

1. 历史记录中已使用的 `V2.0-r1` … `rN` 均视为**轻微**修订号，保留不 retroactive 重编号。
2. 自本规则生效起，**中等**变动优先使用次版本号（如 `V2.1`）；若与现有 `V2.0-rN` 序列并存，须在修订表中明确标注变动等级为**中等**。
3. **重大**变动必须更新「当前版本」主版本号，并在修订摘要中说明影响范围。

#### 0.2.3 index.html 版本号规则

| 变动等级 | 版本号写法 | 示例 |
| --- | --- | --- |
| 重大 | 递增**主版本号** | `V1.x` → `V2.0`（入口流程或信息架构重构） |
| 中等 | 递增**次版本号** | `V1.0` → `V1.1`（交互体、对话页、任务流、新模块） |
| 轻微 | 递增**补丁号** | `V1.1` → `V1.1.1`（样式微调、tooltip 位置、图标等） |

说明：

1. 历史 `index.html V1.0` … `V1.20` 仍按原序号保留；表中用「变动等级」列标注实际等级（早期部分 `V1.x` 实为轻微调整，见 §0.3 逐条标注）。
2. **交互体相关**原型迭代（V1.12 模式切换，V1.15–V1.20 任务流与智能体协作等）均标注为**中等**。
3. 自 `V1.20` 起，**轻微**变动应使用三位版本号补丁位（如 `V1.20.1`）。

#### 0.2.4 后续修订规则

1. 每次修改必须新增一条修订记录，不得覆盖或删除历史记录。
2. 修订时间统一采用北京时间（UTC+8），使用 `YYYY-MM-DD HH:mm` 格式。
3. 每条记录必须填写**变动等级**（重大 / 中等 / 轻微），并与 [§0.2.1](#021-三级变动定义) 定义一致。
4. **重大**变动递增主版本号（如 `V2.x` → `V3.0`）；**中等**变动递增次版本号（如 `V2.0` → `V2.1`）；**轻微**变动使用 `rN` 或补丁号（如 `V2.0-r17`、`V2.0.1`）。
...
7. 若同一次改动同时涉及 PRD 与 `index.html`，两处修订表均需追加记录，且**变动等级应一致**（除非一侧仅为排版同步，可标轻微）。
...
2. 版本号规则（与 [§0.2.3](#023-indexhtml-版本号规则) 一致）：
   - **重大**：主版本递增（如 `V1.x` → `V2.0`）
   - **中等**：次版本递增（如 `V1.0` → `V1.1`）；**交互体 / 任务流 / 多智能体协作**相关改动均属此类
   - **轻微**：补丁号递增（如 `V1.20` → `V1.20.1`）；仅样式、排版、文案微调
3. 修改内容摘要需说明用户可见变化，而非仅列 commit message。
4. 若某次修改同时更新了 PRD 需求范围，PRD 修订记录（0.1）与本表均需分别追加记录，且变动等级应一致。
