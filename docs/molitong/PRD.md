...
> [打开双栏独立滚动阅读版](./PRD.html)：左侧为可上下滑动目录，右侧为可上下滑动 PRD 正文。修改正文后请执行 `npm run sync-prd:molitong`（或 `python3 scripts/sync-prd-molitong.py`）同步到 HTML 阅读页。
...
| V2.0-r34 | 2026-08-13 17:55 | 智能校对结果标注与校对项清单 | 同步 `index.html` V1.37：校对项对齐「校对内容梳理」五类清单；智能校对后编辑器橙色/蓝色错误标注、建议卡片与统计条联动；本次不改变产品需求范围。 |
| V2.0-r35 | 2026-08-13 18:10 | 勘误表弹窗 | 同步 `index.html` V1.38：「查看勘误表」弹窗表格展示标记错误（等级/类型/可疑/建议/详情/状态）及导出；本次不改变产品需求范围。 |
...
7. **每次修改 PRD 正文后，必须执行 `npm run sync-prd:molitong`（或 `python3 scripts/sync-prd-molitong.py`），将最新内容同步到 `PRD.html`。** `docs/molitong/PRD.md` 是唯一编辑源，`PRD.html` 为双栏阅读展示页，不得只改其中一个文件。
...
| index.html V1.37 | 2026-08-13 17:55 | 智能校对结果标注：校对项对齐「校对内容梳理」五类（含专项巡查校对）；编辑器改为仿宋 18px 可编辑区；智能校对后橙色高亮+叹号标记错别字、蓝色下划线标记疑似标点；右侧建议卡片（误报/接受）、已选择全部错误类型与一键接受；统计条联动（全部错误/一般错误/疑似错误等）；查看示例加载刺猬故事 demo。 |
| index.html V1.38 | 2026-08-13 18:10 | 勘误表弹窗：点击「查看勘误表」以表格展示已标记错误（错误等级/类型/可疑内容/建议内容/详细信息红色高亮/处理状态）；示例文本改为《春》节选；支持全选与 CSV 导出。 |
...
- 校对项须覆盖「校对内容梳理」五类：**文字标点差错校对**、**知识性差错校对**、**内容导向风险识别**、**自定义词**、**专项巡查校对**（含两会规范用语校对）。
- 顶部：上传文档、选择校对类型、自定义词库；右侧统计条（今日/单次检测字数、升级）。
- 中部：富文本工具栏（仿宋 18px）+ 可编辑区；底部：复制、清空、导出文档、智能校对；「查看示例」加载刺猬故事 demo 文本。
- 智能校对完成后：
  - 编辑器内 **错别字** 以橙色底 + 黄色叹号角标标记（如「聪明伶俐厉害」）；
  - **疑似标点** 以蓝色下划线标记（如「生活着，它们」中的逗号）；
  - 右侧中间区展示建议卡片（类别标题、原文、建议替换、误报/接受）及「已选择全部错误类型」「一键接受」；
  - 最右侧「校对结果」统计条联动更新（全部错误、一般错误、疑似错误等）。
- 点击「选择校对类型」：弹出「选择校对项」模态，四类+专项规则网格，支持全选、专项设置、取消/确定；Esc 可关闭。
- 点击「查看勘误表」：弹出勘误表模态，以表格汇总已标记错误（错误等级、错误类型、可疑内容、建议内容、详细信息中可疑词红色高亮、处理状态）；支持行勾选与 CSV 导出。
